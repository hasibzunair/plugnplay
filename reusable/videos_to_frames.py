...
Usage:
# python videos_to_frames.py --source PATH_TO_VIDEOS --dest PATH_TO_IMAGE_FOLDER --maxframes 100
"""
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument(
        ...
        type=str,
        default="data/videos",
        help="the destination path, e.g. video-file/dir.",
    )
    parser.add_argument(
        "--dest",
        type=str,
        default="data/frames",
        help="the destination path, e.g. image-file/dir.",
    )

    # for video2frames function
    parser.add_argument(
        "input", default="data/videos.mp4", nargs="?", const=1, help="Input video file"
    )
    parser.add_argument(
        "output",
        default="data/frames",
...
for folder, contents in folder_contents.items():
    for content in contents:
        ...
        dst_file_rename = os.path.join(
            args.dest, f"{os.path.split(args.dest)[-1]}{ct}.jpg"
        )
